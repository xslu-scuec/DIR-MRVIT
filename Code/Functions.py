...
def save_img(I_img,Head, savename,h):
    # affine = np.diag([1, 1, 1, 1])
    new_img = nib.nifti1.Nifti1Image(I_img, Head, header=h)
    nib.save(new_img, savename)


def save_img_nii(I_img,Head,  savename,h):
    # affine = np.diag([1, 1, 1, 1])
    new_img = nib.nifti1.Nifti1Image(I_img, Head, header=h)
    nib.save(new_img, savename)


def save_flow(I_img,Head,  savename,h):
    # affine = np.diag([1, 1, 1, 1])
    new_img = nib.nifti1.Nifti1Image(I_img, Head, header=h)
    nib.save(new_img, savename)
...
class Dataset_epoch(Data.Dataset):
    ...
    def __getitem__(self, step):
        'Generates one sample of data'
        # Select sample
        img_A = load_4d_Brain(self.index_pair[step][0])
        img_B = load_4d_Brain(self.index_pair[step][1])
...
class newVmDataset(Data.Dataset):
    def __init__(self, fixfiles,movfiles):
        # 
        self.fixfiles = fixfiles
        self.movfiles = movfiles

    def __len__(self):
        # 
        return len(self.movfiles)

    def __getitem__(self, index):
        # 
        # 
        img_arr=[]
        fix_img_arr = sitk.GetArrayFromImage(sitk.ReadImage(self.fixfiles[index]))[np.newaxis, ...]
        mov_img_arr = sitk.GetArrayFromImage(sitk.ReadImage(self.movfiles[index]))[np.newaxis, ...]
        img_arr.append(fix_img_arr)
        img_arr.append(mov_img_arr)
        # 
        return img_arr
