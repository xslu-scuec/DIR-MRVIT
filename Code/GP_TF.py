...
# from LapIRN.Code.coordAttention import NewCoordAtt
from Functions import generate_grid_unit
from TransBlock import Tf




class Miccai2020_LDR_laplacian_unit_add_lvl1(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, patch_shape=(40, 40, 8), range_flow=0.4,
                 patch_model=None):
        ...
        self.TransBlock = Tf(self.start_channel * 4, 2, 2).cuda()
        self.grid_1 = generate_grid_unit(self.patch_shape)
        ...
        self.output_lvl1 = self.outputs(self.start_channel * 8, self.n_classes, kernel_size=3, stride=1, padding=1,
                                        bias=False)
        # self.ca1 = NewCoordAtt(self.start_channel * 4, self.start_channel * 4)
        # self.ca2 = NewCoordAtt(self.start_channel * 8, self.start_channel * 8)
    ...
    def forward_once(self, x, y):
        ...
        q1, q2, q3, q4, q5 = self.patch_model(self.down_avg(x), self.down_avg(y))
        q1 = self.up_tri(q1)

        cat_input = torch.cat((x, y), 1)
        # cat_input = self.down_avg(cat_input)
        # cat_input_lvl1 = self.down_avg(cat_input)
        cat_input_lvl1 = cat_input
        down_y = cat_input_lvl1[:, 1:2, :, :, :]

        fea_e0 = self.input_encoder_lvl1(cat_input_lvl1)
        # fea_e0 = self.ca1(fea_e0)
        e0 = self.down_conv(fea_e0)
        e0 = self.TransBlock(e0)
        e0 = e0 + q5
        e0 = self.resblock_group_lvl1(e0)
        e0 = self.up(e0)
        # patch_before = self.ca2(torch.cat([e0, fea_e0], dim=1))
        output_disp_e0_v = self.output_lvl1(torch.cat([e0, fea_e0], dim=1)) * self.range_flow
        compose_output_disp_e0 = output_disp_e0_v + q1
        warpped_inputx_lvl1_out = self.transform(x, compose_output_disp_e0.permute(0, 2, 3, 4, 1), self.grid_1)

        return compose_output_disp_e0, warpped_inputx_lvl1_out, down_y, output_disp_e0_v, e0

    def split_partition(self, x):
        # print('Split')
        b, c, n, h, w = x.shape
        # print(b, c, n, h, w)
        n, h, w = int(n // 2), int(h // 2), int(w // 2)
        x = x.view(b, c, 2, n, 2, h, 2, w)
        # print(x.shape)
        x = x.permute(0, 1, 2, 4, 6, 3, 5, 7).contiguous().view(b, c, 8, n, h, w)
        # print(x.shape)
        # print(x.permute(2, 0, 1, 3, 4, 5).shape)
        return x.permute(2, 0, 1, 3, 4, 5).contiguous()

    def reverse_partition(self, x):
        # print('Reverse')
        s, b, c, n, h, w = x.shape
        # print(s, b, c, n, h, w)
        x = x.permute(1, 2, 0, 3, 4, 5).contiguous()
        x = x.view(b, c, 2, 2, 2, n, h, w)  # 1 3  2 2 2  2 20 20
        # print(x.shape)
        n, h, w = int(n * 2), int(h * 2), int(w * 2)  # 4 40 40
        x = x.permute(0, 1, 2, 5, 3, 6, 4, 7).contiguous().view(b, c, n, h,
                                                                w)  # 1 3 2 2 2 2 20 20 == 1 3 2 2 2 20 2 20 == 1 3 4 40 40
        # print(x.shape)
        return x

    def forward(self, x, y):

        xs = self.split_partition(x)
        ys = self.split_partition(y)

        out = []

        inner_outs = [list(self.forward_once(x, y)) for x, y in zip(xs, ys)]
        # print(len(inner_outs))
        for i in range(5):  # 
            os = [x[i] for x in inner_outs]
            # print('OS', len(os))
            os = torch.stack(os)
            os = self.reverse_partition(os)
            out.append(os)  # 
        # return tuple(out) if self.is_train is True else out[0]

        if self.is_train is True:
            return out[0], out[1], out[2], out[3], out[
                4]  # compose_output_disp_e0, warpped_inputx_lvl1_out, down_y, output_disp_e0_v, e0
        else:
            return out[0]  # output_disp_e0


class Miccai2020_LDR_laplacian_unit_add_lvl2(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, patch_shape=(20, 20, 4), range_flow=0.4):
    ...
    def forward_once(self, x, y):
        ...
        e0 = self.resblock_group_lvl1(e0)
        e0 = self.up(e0)
        # patch_ca = self.ca2(torch.cat([e0, fea_e0], dim=1))
        output_disp_e0_v = self.output_lvl1(torch.cat([e0, fea_e0], dim=1)) * self.range_flow
    ...
    def forward(self, x, y):
        ...
        if x.shape == (1, 1, 48, 48, 8):
            ...
            if self.is_train is True:
                return out[0], out[1], out[2], out[3], out[
                    4]  # output_disp_e0, warpped_inputx_lvl1_out, down_y, output_disp_e0_v, e0
            else:
                return out[0]  # output_disp_e0
        else:
            # x = self.down_avg(x)
...
class Miccai2020_LDR_laplacian_unit_disp_add_lvl1(nn.Module):
    ...
    def forward(self, x, y):
        ...
        fea_e0 = self.input_encoder_lvl1(cat_input_lvl1)
        # fea_e0 = self.ca1(fea_e0)
        # fea_e0 = self.TransBlock(fea_e0)
        e0 = self.down_conv(fea_e0)
        e0 = self.TransBlock(e0)
        e0 = self.resblock_group_lvl1(e0)
        e0 = self.up(e0)
        # before = self.ca2(torch.cat([e0, fea_e0], dim=1))
        output_disp_e0_v = self.output_lvl1(torch.cat([e0, fea_e0], dim=1)) * self.range_flow
        warpped_inputx_lvl1_out = self.transform(x, output_disp_e0_v.permute(0, 2, 3, 4, 1), self.grid_1)
...
class Miccai2020_LDR_laplacian_unit_disp_add_lvl2(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, imgshape=(160, 192, 144), range_flow=0.4,
        ...
        self.patch_model = patch_model_lv2
    ...
    def forward(self, x, y):
        ...
        # p1, p2, p3, p4, p5 = self.patch_model(x, y)
        ...
        patch_cat = torch.cat((y_down, lvl1_disp_up), 1)
        p1, p2, p3, p4, p5 = self.patch_model(warpped_x, patch_cat)
        ...
        # before = self.ca2(torch.cat([e0, fea_e0], dim=1))
        output_disp_e0_v = self.output_lvl1(torch.cat([e0, fea_e0], dim=1)) * self.range_flow
        compose_field_e0_lvl1 = lvl1_disp_up + output_disp_e0_v + p1
...
class Miccai2020_LDR_laplacian_unit_disp_add_lvl3(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # pF_X_Y, pX_Y, pY_4x, pF_xy, _ = self.patch_model(x, y)
        ...
        cat_patch = torch.cat((y, lvl2_disp_up), 1)
        pF_X_Y, pX_Y, pY_4x, pF_xy, _ = self.patch_model(warpped_x, cat_patch)
        ...
        compose_field_e0_lvl1 = output_disp_e0_v + lvl2_disp_up + pF_X_Y

        warpped_inputx_lvl1_out = self.transform(x, compose_field_e0_lvl1.permute(0, 2, 3, 4, 1), self.grid_1)
        # output_disp_e0_v, warpped_inputx_lvl1_out, down_y, output_disp_e0_v, e0
        if self.is_train is True:
...
class stable_multi_resolution_NCC(torch.nn.Module):
    """
    local (over window) normalized cross correlation
    """

    def __init__(self, win=None, eps=1e-5, scale=3):
        super(stable_multi_resolution_NCC, self).__init__()
        self.num_scale = scale
        self.similarity_metric = []

        for i in range(scale):
            self.similarity_metric.append(NCC(win=3))
